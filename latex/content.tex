...
We implement only one class in L2, \code{Poly}, which wraps up by composition \code{GFP}, \code{DUP},
\code{DMP} and \code{SDP} classes of L1, which we call, in this setup, polynomial representations. The
class of L2 implements the union of all methods that are available in L1 classes. If certain
operation is not supported by an underlying representation, \code{OperationNotSupported} exception
is raised. Otherwise, during a method call of \code{Poly} high--level input arguments are converted
...
\code{Poly} implements expression parser, which allows to construct polynomials not only from
...
user to provide additional information manually. For example \code{Poly} can figure out the
...
\code{Poly} are exposed to the top--level via global functions of L3 level. Those functions
...
way. For example, there is a method of L2 and a function of L3, \code{factor\_list()}, which returns
a list of irreducible factors of a polynomial. L3 implements an additional function \code{factor()}
which uses \code{factor\_list()} to compute factorization of a polynomial, but instead of returning
a list, it returns an expression in factored (multiplicative) form.

The \code{Poly} and all functions exposed by L3 level are called the public API of polynomials
manipulation module. The public API is the outcome of \code{from sympy.polys import *} statement. It
...
substituting $15$ for $x$ using \code{subs()} method of \code{Basic} (the root class in SymPy).
This might not seem to be the optimal solution for the problem, because \code{subs()} does not
...
acceptable if \code{subs()} was used as a component of another algorithm, which requires several
thousandths of evaluations? Lets consider a more demanding example. We now use \code{random\_poly()}
function to generate a polynomial of large degree to see how \code{subs()} scales when the size of
...
\end{Verbatim}
\noindent
We used \code{Poly} to obtain polynomials form of $f$ and $g$, arriving with polynomials $F$
and $G$ respectively. We can clearly see, especially in the case of the large degree polynomial,
that \href{http://docs.python.org/library/functions.html\#eval}{\code{eval()}} introduced a significant improvement in execution times. This is not an
...
arithmetics. This may be considered as a cheat, so lets force \code{Poly} to compute with
...
take in to account the construction times of \code{Poly} class instances. Lets check if
this is significant:
...
This is because \code{Poly} class constructor expands the input expression by default and
...
this issue, because the reason for which \code{expand()}, a function which is used for
expanding expressions, is so slow, is because of its bulky implementation, which will
change in near future. Thus, there will be no need to bother with \code{expand} option.

Why \href{http://docs.python.org/library/functions.html\#eval}{\code{eval()}} function of \code{Poly} is so fast? One reason we already know, it
uses an optimized algorithm for its task and it takes advantage over very fast integers.
However, this is only a part of the story. The other is its implementation. \href{http://docs.python.org/library/functions.html\#eval}{\code{eval()}}
...
the first one we will use \code{monomials()} function, which generates all monomials
...
\section{Categories, domains and ground types}\label{thesis-ground}
...
is \code{Ring} category which captures the properties and operations of Ring mathematical structure. Then
there is \code{IntegerRing} category which special--cases \code{Ring}. Finally there are several domains,
\code{ZZ\_sympy}, \code{ZZ\_python} and \code{ZZ\_gmpy}, which implement the interface of \code{IntegerRing}
using appropriate ground types: \code{Integer}, \code{int} and \code{mpz}, respectively.
...
\code{Integer} type are the slowest. This is because, \code{Integer} is a wrapper if Python's \code{int}
type and, thus, it adds significant overhead. From \ref{fig-ground-factor-small} we see that
\code{int} is a little better for small coefficients than gmpy's \code{mpz} type. A reason for this
might be that attribute access for the built--in type is highly optimized, whereas \code{mpz} has
to use more general ways to access attributes. We did not investigate this, so there might be
other explanations for this behaviour. However, for large coefficients \code{mpz} is unbeatable,
because it implements asymptotically faster algorithms for integer arithmetics than \code{int} has.
...
supported by SymPy, there is a need to use SymPy`s \code{Rational} type to implement rational
number domain, because \code{Fraction} type was introduced in Python 2.6.
